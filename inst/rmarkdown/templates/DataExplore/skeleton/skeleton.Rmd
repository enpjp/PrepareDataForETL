...
# Name all columns
Journal_names <- c("Date", "Time", "Common.Name", "Name","Location", "Parent.Location", "Count", "GeoTag", "Gender" ,"Heard.Only", "Notes")
#Note that columns only show when they have data in the observation report. This means that the number o columns can vary!
# Also Name - the scientific name, is always blank.

#my.csv.data <- read.csv (file= file_for_input, col.names = Journal_names, header = TRUE, na.strings = "NA", skip = 0, check.names = TRUE, strip.white = TRUE, blank.lines.skip = TRUE, stringsAsFactors=FALSE)
# Lets read the csv data as is
...
# The Bird Journal Format does not repeat values so we have to fill them in before we can use the data
# Lots of columns so lets use a loop
  for(my.col in 1:13) {
  test.place <- fill(my.data, names(my.data)[my.col])
  my.data <- test.place
  }
rm(test.place) # Remove the scratchpad
...
write.csv(NatureSpot, file = "Naturespot_upload.csv", row.names=FALSE )
#rm(NatureSpot,data)
gc()
